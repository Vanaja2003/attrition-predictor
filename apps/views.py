...
def Attrition_rate_finder(request):
    ...
    if request.method == 'POST':
        try:
            ...
            emp_group = request.POST.get('emp_group', '')
            function = request.POST.get('function', '')
            gender = request.POST.get('gender', '')
            tenure_group = request.POST.get('tenure_group', '')
            experience = request.POST.get('experience', '')
            age = request.POST.get('age', '')
            marital = request.POST.get('marital_status', '')
            hiring = request.POST.get('hiring_source', '')
            promoted = request.POST.get('promoted', '')
            job = request.POST.get('job_role_match', '')
            ...
            print(f"marital={marital}, hiring={hiring}, promoted={promoted}, job={job}")
            ...
            try:
                ...
                raise ValueError("Please enter valid numeric values for Experience and Age")

            # Convert boolean fields
            promoted_bool = promoted.lower() == 'yes'
            job_bool = job.lower() == 'yes'

            prediction = Finder(name, location, emp_group, function, gender, tenure_group,
                              experience, age, marital, hiring, promoted, job)
            ...
            print(f"Error in Attrition_rate_finder: {str(e)}")
            print(traceback.format_exc())
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({
                    'error': str(e),
